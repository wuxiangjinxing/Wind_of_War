...
# #Formations Scripts	  
		...
		(try_for_range, ":weap_group", 0, 4),
			...
			(try_for_agents, ":agent"),
				...
				(try_begin),
					...
					(item_get_weapon_length, ":weapon_length", ":agent_weapon"),
  ...
  ("get_default_formation", [
	...
	(try_begin),
		(this_or_next|eq, ":ffaction", fac_kingdom_13),	#Swadians
		(this_or_next|eq, ":ffaction", fac_kingdom_7),	#Swadians
		(this_or_next|eq, ":ffaction", fac_kingdom_4),	#Swadians
		(this_or_next|eq, ":ffaction", fac_kingdom_10),	#Swadians
		(this_or_next|eq, ":ffaction", fac_kingdom_5),	#Swadians
		(this_or_next|eq, ":ffaction", fac_kingdom_11),	#Swadians
		(eq, ":ffaction", fac_kingdom_1),	#Swadians
		(assign, reg0, formation_shield),
	(else_try),
		(this_or_next|eq, ":ffaction", fac_hospitalier_knights),	
		(this_or_next|eq, ":ffaction", fac_kingdom_14),	
		(this_or_next|eq, ":ffaction", fac_kingdom_8),	
	...
	(else_try),
		(this_or_next|eq, ":ffaction", fac_kingdom_9),	#Sarranid
		(this_or_next|eq, ":ffaction", fac_kingdom_12),	#Sarranid
	...
	(else_try),
		...
		(assign, reg0, formation_ranks),
	(else_try),
	  (this_or_next|eq, ":ffaction", fac_swiss),
		(eq, ":ffaction", fac_scotland),	#independent player
	...
	(try_begin), #CABA additions
		...
		(try_begin),
			...
			(try_for_range, ":item_slot", ek_item_0, ek_head),
				...
				(this_or_next|neg|item_has_property, ":item", itp_two_handed),
  ...
  ("store_battlegroup_data", [
	...
	(try_for_agents, ":cur_agent"),
		...
		(try_begin),
			...
			(item_get_swing_damage, ":swing_damage", ":cur_weapon"),
			(item_get_thrust_damage, reg0, ":cur_weapon"),
			(ge, ":swing_damage", reg0),
  ...
  ("formation_battle_tactic_init_aux",
    [
      ...
      (try_begin),
        ...
        (team_give_order, ":team_no", grc_archers, mordr_advance),
      (else_try),
        (this_or_next|eq, ":battle_tactic", btactic_cav_charge),
...
#formations code
          ...
          (try_begin),
            ...
            (call_script, "script_cf_battlegroup_valid_formation", ":team_no", grc_cavalry, formation_ranks),
            (store_add, ":slot", slot_team_d0_formation, grc_cavalry),
            (team_set_slot, ":team_no", ":slot", formation_ranks),
            (store_add, ":slot", slot_team_d0_formation_space, grc_cavalry),
            (team_set_slot, ":team_no", ":slot", 0),
          (else_try),
            (call_script, "script_cf_battlegroup_valid_formation", ":team_no", grc_cavalry, formation_wedge),
